    ...
    <div class="py-12">
        <div class="max-w-md mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-8">
                <form method="POST" action="{{ route('secretaria.asignaturas.update', $asignatura) }}" class="space-y-4">
                    @csrf
                    @method('PATCH')
                    <div>
                        <label for="nombre" class="block font-medium mb-1 dark:text-white">Nombre de la Asignatura</label>
                        <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $asignatura->nombre) }}" class="w-full rounded-md border-gray-300 shadow-sm dark:bg-gray-700 dark:text-white focus:ring-[#0099a8] focus:border-[#0099a8]" required>
