        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-12">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                @forelse($apelaciones as $apelacion)
                    <a href="{{ route('estudiante.apelaciones.show', ['apelacion' => $apelacion, 'estado' => $estado]) }}"
                        class="{{
                        ...
                        <span class="bg-[#0099a8] text-white text-xs px-2 py-1 rounded font-semibold">{{ ucfirst($apelacion->estado) }}</span>
                        @elseif ($apelacion->estado === 'aprobada')
                        <span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded font-semibold">{{ ucfirst($apelacion->estado) }}</span>
                        @elseif ($apelacion->estado === 'rechazada')
                        <span class="bg-red-100 text-red-800 text-xs px-2 py-1 rounded font-semibold">{{ ucfirst($apelacion->estado) }}</span>
                        @endif
                            </p>
                    </a>         
